...
| Fast retry on rate-limit failure (T4.11, post-ship) | `threshold.py` gained a `pending_failure` flag on its existing per-session trigger file; `worker.py` sets it after a retryable provider failure and clears it after a success; `should_trigger()` fires unconditionally while it's set. | User-reported friction: a real `QuotaExhausted` hit during T4.10's own measurement appeared to "abort" logging until the next full `SF_TURN_THRESHOLD` worth of turns. A sleep-and-retry loop inside `worker.py` was considered and rejected — spec.md's own SHALL requirement is "retry on the next trigger," not internally, and a sleep loop would hold the per-session lock (T4.4) longer, silently absorbing a real concurrent trigger that arrives while it's asleep. The actual bug was that "next trigger" wasn't tied to whether the *previous* attempt had succeeded. Fixed at the trigger-frequency layer only; `summarise_with_failover`'s retry/failover semantics are untouched (re-confirmed: "exactly one fallback call" still holds after the fix). |
| `.env` file support (T4.12, post-ship, reverses spec.md's original "environment variables only") | `config.load_dotenv(cwd)`: a small, dependency-free `KEY=value` parser, called once early in `worker.run()` and `worker.run_over_transcript()`. Merges into `os.environ` via `setdefault` (a real env var always wins), and warns (logged, not raised) if `.env` exists without an obvious covering `.gitignore` line. | User-reported friction, again post-ship: the original "set a persistent OS environment variable" path requires `setx` *and* a full Claude Code restart before the new value is visible to the hook chain — real ceremony for a tool meant to need none, and most developers already carry a project `.env` from other tooling. Rejected: adopting `python-dotenv` (would break the "standard library only, nothing to `pip install`" property that's held since T2.1) and auto-writing `.env` into the repo's root `.gitignore` (too presumptuous a side effect on a file the tool doesn't own, unlike `.decisions/.gitignore`, which it does). The unprotected-`.env` warning exists because a leaked provider key is a real, plausible outcome of making this easier, not a hypothetical. |
| `no_key`/`cap_reached` reset (T4.13, post-ship bugfix) | Both flags cleared in `_apply_update` the moment a call is attempted, not just when one succeeds. | Both flags were previously set-only, never reset, so a digest carried a stale "no provider key configured" line forward forever once one update happened to see no key — even after a later update succeeded with a real one. Found from a real digest the user reported directly, immediately after T4.12 shipped. `cap_reached` got the same fix for the same reason, not just `no_key`: `SF_CALL_CAP` is read fresh every call, so it is not actually as monotonic as it first appears. |
| Session title in the digest header (T4.14, post-ship) | `transcript/claude_code.py` recognises `ai-title`/`custom-title` records (Claude Code's own session-naming mechanism, confirmed directly against this repo's own real transcript); threaded through `Transcript` → `Delta` → `Digest.ai_title`/`Digest.custom_title`, with `Digest.title` resolving `custom_title or ai_title`. `render.py`'s header shows the title as the H1 when one exists, with the session id demoted to a subtitle line; falls back to the original `# Session digest: `id`` format exactly when no title exists yet. `aggregate.py`'s per-session group header shows it too. | User asked directly whether the UUID-named digests could show "the actual session name" instead, explicitly authorizing a spec.md scope addition if the answer was yes. It was: real transcripts carry these record types (verified against this project's own live session, not assumed). **The filename itself deliberately stays keyed on the session id** — `output/state.py`'s sidecar, `threshold.py`'s trigger file and `output/lock.py`'s lock file are all keyed on it too, a title can be absent (a session killed before Claude Code assigns one), renamed mid-session, or collide between two unrelated sessions, and `aggregate.py`'s cross-session deduplication and chronological ordering both depend on session id being a stable, unique, append-only key — none of which a title can promise. Title merging deliberately does *not* follow `branch`/`session_started`'s existing "first seen, sticky" pattern: a title can legitimately be renamed, so `merge.py` takes the latest non-null value seen instead, unlike the session-invariant facts that pattern was built for. |
| Primary model switched to Flash-Lite (T4.15, post-ship) | `SF_MODEL_PRIMARY` default → `gemini-flash-lite-latest` (was `gemini-flash-latest`). | User-directed, from their own AI Studio dashboard: `gemini-3.6-flash` (what `gemini-flash-latest` resolved to) sits at 5 RPM against Flash-Lite's 15 RPM / 250k TPM — a real, account-sourced figure, more authoritative than anything publicly documented (Google no longer publishes static per-tier numbers, confirmed again during this same research pass). Directly relevant given T4.10 already established request-rate as this tool's real binding constraint, not spend. Kept the same "-latest alias, never a pinned version" discipline that `gemini-flash-latest` was chosen for at T3.9, for the identical reason: a version string goes stale, an alias doesn't. **Not yet independently confirmed against a real call** the way the original switch was at T3.9 — this project's own history (the stale `gemini-2.5-flash` bug) is the reason that gap is stated here rather than left implicit; asked the user to confirm with their own next real session rather than assumed working. |
