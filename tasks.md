...
- [x] **T4.14** *(added post-ship, not in the original plan)* Show Claude Code's own session title in the digest header instead of only the raw session id.
  - **Where this came from**: the user asked directly whether digests could be identified by "the actual session name" rather than a UUID, explicitly authorizing a spec.md scope addition if it turned out to be possible — feasibility was investigated first, not assumed.
  - **Investigated before promising anything**: grepped this project's own real, live transcript (this very session) for a title field. Found two real record types Claude Code writes into the transcript itself: `{"type": "custom-title", "customTitle": "..."}` (set when a title is user-edited) and `{"type": "ai-title", "aiTitle": "..."}` (auto-generated). Confirmed directly against this session's own transcript: `ai_title` = "Build decision tracking hook for coding sessions", `custom_title` = "Decision tracking hook for coding assistants" — the exact string the user quoted as an example. A third candidate, `last-prompt` (the literal first user message, truncated with an ellipsis), was considered and rejected as a fallback — it is raw prompt text, not a title, and out of scope for this addition.
  - **Filename deliberately NOT renamed to the title** — the one meaningful design judgment call here, stated plainly rather than silently decided: a title can be absent (a session killed after one or two turns, before Claude Code assigns one), can be renamed mid-session, and two unrelated sessions can end up with the same or a very similar title. `output/state.py`'s sidecar, `threshold.py`'s trigger file and `output/lock.py`'s lock file are all keyed on the session id, and `aggregate.py`'s cross-session deduplication depends on that id being a stable, collision-free, append-only key — none of which a human-editable title can guarantee. Renaming the file itself would risk exactly the kind of data loss (two sessions silently overwriting one another) this project has treated as unacceptable since T4.1's write gate. The title is shown as *content* instead — the safe version of the same outcome the user actually wants (identify a session at a glance).
  - **Implementation**: `transcript/claude_code.py` recognises the two record types; `transcript/events.py`'s `Transcript` gained `ai_title`/`custom_title` fields; `extract/delta.py`'s `Delta` threads them through; `digest/model.py`'s `Digest` gained the same two fields plus a `title` property (`custom_title or ai_title`); `digest/merge.py::accumulate_session_stats` folds them in with **latest-value-wins** semantics, deliberately *not* the `branch`/`session_started` pattern of "first seen, sticky" — a title can legitimately be renamed, so a later `custom-title` record correctly overrides an earlier `ai-title` one, verified directly (see below). `output/state.py` persists both fields in the sidecar. `digest/render.py`'s header becomes `# {title}` / `` _Session `{id}`_ `` when a title exists, falling back to exactly the original `` # Session digest: `{id}` `` format when it does not. `aggregate.py`'s per-session group header shows the title too — arguably the highest-value spot, since the merged report is the stated deliverable and a report organised by name reads far better than one organised by UUID.
  - **Verified directly** (`test_session_title.py`, 12 checks): both record types parsed correctly from a fabricated transcript and cross-checked against this project's own real one; `Digest.title` precedence (custom over ai, ai alone, neither); a full `worker.run()` end to end shows the title as the H1 with the session id demoted to a subtitle, the digest *file* still named by session id; a `custom-title` arriving in a later update correctly overrides an earlier `ai-title`; a session with no title at all renders the exact pre-T4.14 header, byte for byte; the aggregate's group header carries the (overridden) title alongside the session id. Full regression sweep re-run clean: every existing scratch test (`test_stale_no_key.py` through `test_prompt.py`) plus `compileall`/`ruff check --select F,E9`/`tests/ci_smoke.py` — checked specifically for positional-argument breakage from the new dataclass fields (`Transcript`/`Delta`/`Digest` are all constructed by keyword everywhere in `src/`, confirmed by grep, so field insertion position was never a risk).
