...
- [ ] All tasks in all phases checked off
- [ ] Every acceptance criterion in `spec.md` § acceptance criteria verified on Windows
- [ ] A digest exists for a session that was killed with Ctrl+C
- [ ] `git status` in a repository that has run the tool shows no untracked digest files
- [ ] `--dry-run` output for a real transcript contains no file contents, no command output, no reasoning text, and nothing from a non-human turn
- [ ] A project with the opt-out marker transmits nothing and still produces a digest
- [ ] The aggregate across the corpus is a document a report could actually be written from
- [ ] The README states Windows-only above the fold, names both providers as recipients, and publishes a measured per-session cost and call count
- [ ] `VERIFICATION.md` has been run end to end with zero unsupported claims
